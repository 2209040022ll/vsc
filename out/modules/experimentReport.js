...
exports.HutbExperimentReportGenerator = void 0;
exports.createExperimentReportData = createExperimentReportData;
exports.extractExperimentParameters = extractExperimentParameters;
exports.extractInlineResultSummary = extractInlineResultSummary;
exports.buildMarkdownReport = buildMarkdownReport;
exports.buildDocxDocumentXml = buildDocxDocumentXml;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const pythonCallParser_1 = require("../utils/pythonCallParser");
const scenePreview_1 = require("./scenePreview");
function createExperimentReportData(scriptText, scriptPath) {
    return {
        title: 'HUTB 仿真实验报告',
        generatedAt: new Date().toISOString(),
        scriptPath,
        scene: (0, scenePreview_1.extractScenePreviewModel)(scriptText, scriptPath),
        parameters: extractExperimentParameters(scriptText),
        resultSummary: extractInlineResultSummary(scriptText)
    };
}
function extractExperimentParameters(scriptText) {
    const parameters = [];
    for (const call of (0, pythonCallParser_1.findApiCalls)(scriptText)) {
        ...
        switch (call.funcName) {
            case 'load_scene':
                parameters.push({
                    name: '场景文件',
                    value: stringifyValue((0, pythonCallParser_1.parseLiteralValue)(parsed.positional[0] ?? parsed.named.get('scene_path'))),
                    line: call.line + 1
                });
                break;
            case 'set_weather':
                parameters.push({
                    name: '天气',
                    value: [
                        stringifyValue((0, pythonCallParser_1.parseLiteralValue)(parsed.positional[0] ?? parsed.named.get('weather_type'))),
...
function extractInlineResultSummary(scriptText) {
...
function buildMarkdownReport(data) {
    ...
    return [
        ...
        '## 实验参数',
        '',
        ...tableSection(['参数', '取值', '来源行'], data.parameters.map(item => [item.name, item.value, String(item.line)])),
        '',
        '## 实验结果',
        '',
        ...tableSection(['指标', '值'], resultRows.length > 0
            ? resultRows.map(([name, value]) => [name, stringifyValue(value)])
            : [['结果数据', '未在脚本或输出中识别到 HUTB_RESULT JSON']]),
        ''
    ].join('\n');
}
function buildDocxDocumentXml(data) {
    const rows = [
        ...
        tableXml(['参数', '取值', '来源行'], data.parameters.map(item => [item.name, item.value, String(item.line)])),
        paragraph('实验结果', 'Heading1'),
        tableXml(['指标', '值'], Object.entries(data.resultSummary).length > 0
...
class HutbExperimentReportGenerator {
    constructor(context) {
        this.context = context;
    }
    async generateFromActiveDocument() {
        const editor = vscode.window.activeTextEditor;
        if (!editor || editor.document.languageId !== 'python') {
            vscode.window.showWarningMessage('请先打开一个 Python 仿真实验脚本。');
            return;
        }
        const defaultFormat = vscode.workspace.getConfiguration('hutb.report').get('defaultFormat', 'markdown');
        const formatItems = [
        ...
        const selected = await vscode.window.showQuickPick(formatItems, { placeHolder: '选择实验报告导出格式' });
        if (!selected) {
            return;
        }
        ...
        vscode.window.showInformationMessage(`HUTB 仿真实验报告已生成：${filePath}`, '打开报告').then(result => {
            if (result === '打开报告') {
                vscode.env.openExternal(vscode.Uri.file(filePath));
            }
        });
    }
    resolveOutputDir(workspaceRoot) {
        const configured = vscode.workspace.getConfiguration('hutb.report').get('outputDir', '');
        if (configured) {
...
exports.HutbExperimentReportGenerator = HutbExperimentReportGenerator;
async function writeDocxReport(filePath, data) {
...
function namedValue(raw, label) {
    const value = stringifyValue((0, pythonCallParser_1.parseLiteralValue)(raw));
