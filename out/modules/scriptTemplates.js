...
exports.HutbScriptTemplateManager = exports.BUILTIN_SIMULATION_TEMPLATES = void 0;
exports.sanitizePythonFileName = sanitizePythonFileName;
exports.listTemplateItems = listTemplateItems;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const CUSTOM_TEMPLATE_KEY = 'hutb.customSimulationTemplates';
exports.BUILTIN_SIMULATION_TEMPLATES = [
    {
...
function sanitizePythonFileName(input) {
...
function listTemplateItems(customTemplates = []) {
    return [
        ...exports.BUILTIN_SIMULATION_TEMPLATES,
        ...customTemplates.map(template => ({ ...template, custom: true }))
    ];
}
class HutbScriptTemplateManager {
    constructor(context) {
        this.context = context;
    }
    async createScript(targetUri) {
        ...
        if (exists) {
            const overwrite = await vscode.window.showWarningMessage(`文件已存在：${fileUri.fsPath}`, '覆盖');
            if (overwrite !== '覆盖') {
                return;
            }
        }
    ...
    async saveActiveFileAsCustomTemplate() {
    ...
    getCustomTemplates() {
        return this.context.globalState.get(CUSTOM_TEMPLATE_KEY, []);
    }
    async resolveTargetDirectory(targetUri) {
        if (targetUri?.fsPath) {
...
exports.HutbScriptTemplateManager = HutbScriptTemplateManager;
//# sourceMappingURL=scriptTemplates.js.map