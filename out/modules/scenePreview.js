...
exports.HutbScenePreviewProvider = void 0;
exports.parsePointLiteral = parsePointLiteral;
exports.extractScenePreviewModel = extractScenePreviewModel;
const vscode = __importStar(require("vscode"));
const pythonCallParser_1 = require("../utils/pythonCallParser");
const DEFAULT_POINT = { x: 0, y: 0, z: 0 };
function parsePointLiteral(raw) {
    if (!raw) {
        return undefined;
    }
...
function extractScenePreviewModel(text, scriptName) {
    const model = {
        scriptName,
        vehicles: [],
        sensors: [],
...
function readObstacle(call, index) {
    const parsed = (0, pythonCallParser_1.parseArguments)(call.argsText);
    return {
...
function readStringArg(raw) {
    const value = (0, pythonCallParser_1.parseLiteralValue)(raw);
...
class HutbScenePreviewProvider {
    constructor() {
    ...
    refresh(document = vscode.window.activeTextEditor?.document) {
        if (this.disposed) {
            return;
        }
...
exports.HutbScenePreviewProvider = HutbScenePreviewProvider;
HutbScenePreviewProvider.viewType = 'hutb.scenePreview';
function getNonce() {
...
//# sourceMappingURL=scenePreview.js.map