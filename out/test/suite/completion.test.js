...
suite('代码补全测试', () => {
    ...
    test('输入 hutb. 应触发补全', async () => {
        ...
        const completions = await vscode.commands.executeCommand('vscode.executeCompletionItemProvider', doc.uri, position, '.', 50);
    ...
    test('已弃用函数应有弃用标记', async () => {
        ...
        if (deprecatedItem) {
            assert.ok(deprecatedItem.tags?.includes(vscode.CompletionItemTag.Deprecated), 'init_sensor 应有弃用标记');
        }
    });
});
