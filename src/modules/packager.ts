...
interface PackConfig {
    vscode: { source: string; version: string; portableMode: boolean };
    extensions: string[];
    python: { source: string; version: string; packages: string[] };
    startScript: { windows: string };
    output: { namePattern: string; format: string };
    sampleProject: { include: boolean; path: string };
}

export class HutbPackager {
    ...
    public async pack() {
        ...
        const vscodePath = pluginConfig.get<string>('vscodePath', '');
        const pythonEnvPath = pluginConfig.get<string>('pythonEnvPath', '');

        if (!vscodePath || !pythonEnvPath) {
            const result = await vscode.window.showWarningMessage(
                '请先在设置中配置 VSCode 便携版路径和 Python 环境路径。',
                '打开设置'
            );
            if (result === '打开设置') {
            ...
            async (progress, token) => {
                try {
                    ...
                    await this.copyDirectory(
                        ...
                        path.join(extensionsDir, 'hutb-simulator-dev')
                    );
    ...
    private generateStartScript(targetDir: string) {
...
`;
    ...
    private generateVSCodeSettings(targetDir: string) {
    ...
    private generateSampleProject(targetDir: string) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                vscode.window.showWarningMessage(
                    `ZIP 压缩需要 archiver 依赖，请运行 npm install archiver。打包目录已准备好: ${sourceDir}`
                );
                resolve();
            }
        });
